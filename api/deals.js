export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. Shape all deals, then filter out any with unknown/legacy stage labels
    const EXCLUDED_STAGES = new Set(['Appointment scheduled']);

    const deals = allDeals
      .map(function(d) {
        const stageLabel = stageMap[d.properties.dealstage] || d.properties.dealstage || 'Unknown';
        return {
          id: d.id,
          ...
          isClosedLost: stageLabel === 'Closed lost',
      ...
      .filter(function(d) { return !EXCLUDED_STAGES.has(d.stage); });
